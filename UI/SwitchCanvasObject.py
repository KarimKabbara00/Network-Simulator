...
class SwitchCanvasObject:
    ...
    def __init__(self, canvas, block_name, icon, class_object):
        ...
        self.submenu.add_command(label="Terminal", command=self.menu_switch_cli)
        self.submenu.add_cascade(label="Disconnect", menu=self.disconnect_menu)
        self.submenu.add_separator()
        self.submenu.add_command(label="Delete Switch", command=self.menu_delete)
        # Submenu Stuff
                ...
                for j in range(line_count[0]):
                    ...
                    if line:
                        ...
                        if 0 <= abs(self.canvas.canvasx(event_x) - self.canvas.coords(line)[0]) <= 30 and 0 <= abs(
                            ...
                            if event:
                                self.canvas.itemconfig(l1, fill=hf.get_color_from_op(self.line_interface_relations[line[0]][0].get_is_operational()))
                                self.canvas.itemconfig(l2, fill=hf.get_color_from_op(self.line_interface_relations[line[0]][1].get_is_operational()))
                            ...
                            if event:
                                self.canvas.itemconfig(l2, fill=hf.get_color_from_op(self.line_interface_relations[line[0]][0].get_is_operational()))
                                self.canvas.itemconfig(l1, fill=hf.get_color_from_op(self.line_interface_relations[line[0]][1].get_is_operational()))
