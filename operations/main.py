...
def create_menu(canvas_obj, master):
    ...
    file_menu.add_command(label="Save", command=lambda c=canvas_obj, m=master: ls.save_current_file(m, c))
    file_menu.add_command(label="Save as New File", command=lambda c=canvas_obj, m=master: ls.save_as_new_file(m, c))
...
# TODO:
#   - DHCP NAK, DECLINE, When client broadcasts REQUEST -> other DHCP servers revoke offered IP address to that ch_address/t_id
